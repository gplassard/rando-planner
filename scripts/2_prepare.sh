...
jq  '.features | map({id: .id, name: .properties.name, from: .properties.from, to: .properties.to, bbox: .properties.bbox})' data/rando.geojson > data/prepared/hiking-routes.json
